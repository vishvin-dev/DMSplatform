...
const ApprovalProcessName = () => {

    const [modal_list, setmodal_list] = useState(false);
    const [edit_update, setedit_update] = useState(false);
    const [checked, setChecked] = React.useState(false);
    const [buttonval, setbuttonval] = useState('Add Approval process');
    ...
    const tog_list = () => {
        ...
        setbuttonval('Add Approval Process');
        setSubmitVal('Save');
        setedit_items([]);
    ...
    async function getOnLoadingData() {
        ...
        let response;
        response = getAllApprovalProcess();
        var allProcess = await response;
        var usernm = obj.data.username;
        setData(allProcess);
        setDataBk(allProcess);
        setUserName(usernm);
        setedit_update(false);
        setedit_items([])
        setbuttonval('Add Approval Process')
    ...
    const updateRow = async (item) => {
        ...
        setbuttonval('Update Approval Process');
        setSubmitVal('Update');
        setmodal_list(!modal_list);
    }
    ...
    const columns = useMemo(
        () => [
            {
                header: 'ApprovalProcessName',
                accessorKey: 'approvalProcessName',
                disableFilters: false,
                enableColumnFilter: false,
            },
            {
                header: 'ApprovalProcessCode',
                accessorKey: 'approvalProcessCode',
                disableFilters: false,
                enableColumnFilter: false,
            },
    ...
    const validation = useFormik({
        ...
        initialValues: {
            approvalProcessId: edit_items.approvalProcessId || "",
            approvalProcessName: edit_items.approvalProcessName || "",
            approvalProcessCode: edit_items.approvalProcessCode || "",

            isDisabled: false,
            requestUserName: ''
        },
        validationSchema: Yup.object({
            approvalProcessName: Yup.string().required("Please Enter Approval Process Name"),
            approvalProcessCode: Yup.string().required("Please Enter Approval Process Code"),
        ...
        onSubmit: async (values) => {
            ...
            try {
                if (edit_update === true) {
                    response = putApprovalProcess({
                        approvalProcessId: edit_items.approvalProcessId,
                        approvalProcessName: values.approvalProcessName,
                        approvalProcessCode: values.approvalProcessCode,
                        isDisabled: !checked,
                        requestUserName: username
                    });
                } else {
                    response = postApprovalProcess({
                        approvalProcessName: values.approvalProcessName,
                        approvalProcessCode: values.approvalProcessCode,
                        isDisabled: false,
                        requestUserName: username
                ...
                if (data) {
                    ...
                    tog_list();
                    getOnLoadingData();
                    values.approvalProcessName = "";
                    values.approvalProcessCode = "";
    ...
    const filter = (e) => {
        ...
        if (keyword !== '') {
            const results = filterData?.filter((d) => {
                return d.approvalProcessName.toLowerCase().includes(keyword.toLowerCase());
            });
            setData(results);
        } else {
            setData(databk);
    ...
    document.title = "Approval Process Name | eSoftDigital Platform";
    ...
    return (
        <React.Fragment>
            ...
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Approval Process" pageTitle="Material"/>
                    <Row>
                        <Col lg={12}>
                            <Card>
                                    ...
                                    <Row className="g-4 mb-3">
                                        <Col className="col-sm">
                                            <div className="d-flex ">
                                                <div className="search-box col-sm-2">
                                                    <input
                                                        type="text" maxLength={100}
                                                        className="form-control"
                                                        id="searchResultList"
                                                        placeholder="Search for Approval Process..."
                                                        onKeyUp={(e) => filter(e)}
                                                    />
                                        ...
                                        <Col className="col-sm-auto justify-content-sm-end">
                                            <div>
                ...
                <form className="tablelist-form"
                    ...
                    <ModalBody>
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="validationCustom01">Approval Process Name</Label>
                                    <Input
                                        ...
                                        value={validation.values.approvalProcessName || ""}
                                        invalid={
                                            validation.touched.approvalProcessName &&
                                            validation.errors.approvalProcessName
                                                ? true
                                                : false
                                        }
                                    />
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="validationCustom01">Approval Process Code</Label>
                                    <Input
                                        name="approvalProcessCode"
                                        placeholder="Enter Approval Process Code"
                                        type="text"
                                        className="form-control"
                                        id="validationCustom01"
                                        onChange={validation.handleChange}
                                        onBlur={validation.handleBlur}
                                        value={validation.values.approvalProcessCode || ""}
                                        invalid={
                                            validation.touched.approvalProcessCode &&
                                            validation.errors.approvalProcessCode
                                                ? true
                                                : false
                                        }
                                    />
...
export default ApprovalProcessName;