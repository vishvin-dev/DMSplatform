...
import {
    postMaterialReceiveModeId,
    putMaterialReceiveModeId,
    getAllMaterialReceiveModeId,
...
const MaterialReceiveMode = () => {


    const [modal_list, setmodal_list] = useState(false);
    const [edit_update, setedit_update] = useState(false);
    const [checked, setChecked] = React.useState(false);
    const [buttonval, setbuttonval] = useState('Add Material Receive Mode');
    ...
    const tog_list = () => {
        ...
        setbuttonval('Add Material Received Mode');
        setSubmitVal('Save')
        setedit_items([]);
    ...
    async function getOnLoadingData () {
        ...
        let response;
        response = getAllMaterialReceiveModeId();
        ...
        setedit_update(false);
        setedit_items('')
        setbuttonval('Add Material Receive Mode')
        setSubmitVal('Save')
    }
    const updateRow = async (item) => {
        ...
        setbuttonval('Update Material Receive Mode')
        setSubmitVal('Update')
        setmodal_list(!modal_list);
    }
    ...
    const columns = useMemo(
        () => [
            {
                header: 'MaterialReceiveModeName',
                accessorKey: 'materialReceiveModeName',
                disableFilters: false,
                enableColumnFilter: false,
            },
            {
                header: 'MaterialReceiveCode',
                accessorKey: 'materialReceiveModeCode',
                disableFilters: false,
                enableColumnFilter: false,
            },
            ...
            {
                header: 'Action',
                accessorKey: 'materialReceiveModeId',
                disableFilters: false,
                enableColumnFilter: false,
                cell: (data) => {
    ...
    const validation = useFormik({
        ...
        initialValues: {
            ...
            materialReceiveModeName: edit_items.materialReceiveModeName || "",
            materialReceiveModeCode: edit_items.materialReceiveModeCode || "",

            isDisabled: false,
            requestUserName: ''
        },
        validationSchema: Yup.object({
            materialReceiveModeName: Yup.string().required("Please Enter Material Receive Mode Name"),
            materialReceiveModeCode: Yup.string().required("Please Enter Material Receive Mode Code "),
        ...
        onSubmit: async (values) => {
            ...
            try {
                if (edit_update === true) {
                    response = putMaterialReceiveModeId({
                        materialReceiveModeId: edit_items.materialReceiveModeId,
                        materialReceiveModeName: values.materialReceiveModeName,
                        materialReceiveModeCode: values.materialReceiveModeCode,
                        isDisabled: !checked,
                        requestUserName: username
                    });
                } else {
                    response = postMaterialReceiveModeId({
                        materialReceiveModeName: values.materialReceiveModeName,
                        materialReceiveModeCode: values.materialReceiveModeCode,
                        isDisabled: false,
                        requestUserName: username
                ...
                if (data) {
                    ...
                    tog_list();
                    getOnLoadingData();
                    values.materialReceiveModeName = "";
                    values.materialReceiveModeCode = "";
    ...
    const filter = (e) => {
        ...
        if (keyword !== '') {
            const results = filterData?.filter((d) => {
                return d.materialReceiveModeName.toLowerCase().includes(keyword.toLowerCase());
            });
            setData(results);
        } else {
            setData(databk);
    ...
    document.title = "Material Receive Mode | eSoftDigital Platform";
    ...
    return (
        <React.Fragment>
            ...
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Material Receive Mode" pageTitle="Material"/>
                    <Row>
                        <Col lg={12}>
                            <Card>
                                        ...
                                        <Col className="col-sm-2">
                                            <div className="search-box ms-2">
                                                <input
                                                    ...
                                                    id="searchResultList"
                                                    placeholder="Search for Material Receive Mode..."
                                                    onKeyUp={(e) => filter(e)}
                                                />
                ...
                <form className="tablelist-form"
                    ...
                    <ModalBody>
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="validationCustom01">Material Receive Mode Name</Label>
                                    <Input
                                        name="materialReceiveModeName"
                                        placeholder="Enter Material Receive Mode Name"
                                        type="text"
                                        maxLength={75}
                                        className="form-control"
                                        ...
                                        value={validation.values.materialReceiveModeName || ""}
                                        invalid={
                                            validation.touched.materialReceiveModeName &&
                                            validation.errors.materialReceiveModeName
                                                ? true
                                                : false
                                        }
                                    />
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="validationCustom01">Material Receive Mode Code</Label>
                                    <Input
                                        name="materialReceiveModeCode"
                                        placeholder="Enter Material Receive Mode Code"
                                        type="text"
                                        maxLength={50}
                                        className="form-control"
                                        id="validationCustom01"
                                        onChange={validation.handleChange}
                                        onBlur={validation.handleBlur}
                                        value={validation.values.materialReceiveModeCode || ""}
                                        invalid={
                                            validation.touched.materialReceiveModeCode &&
                                            validation.errors.materialReceiveModeCode
                                                ? true
                                                : false
                                        }
                                    />
...
export default MaterialReceiveMode;