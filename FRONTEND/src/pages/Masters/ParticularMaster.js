...
import {
    updateParticularMaster,
    saveParticularMaster,
    viewParticularMaster
...
const ParticularMaster = () => {
    const [modal_list, setmodal_list] = useState(false);
    const [edit_update, setedit_update] = useState(false);
    const [checked, setChecked] = React.useState(false);
    const [buttonval, setbuttonval] = useState('Add Particular Master');
    ...
    const [sessClient, setSessClient] = useState('');
    const [checkedText, setCheckedText] = React.useState('');
    const tog_list = () => {

        setedit_update(false);
        setedit_items([])
        setbuttonval('Add Particular Master');
        setSubmitVal('Save');
    ...
    async function getOnLoadingData() {
        ...
        console.log(obj);
        var country=obj.data.countryId;
        var client=obj.data.clientId;
        setScountry(country);
        setSessClient(client);

        let response;
        response = viewParticularMaster(client);
    ...
    const updateRow = async (item) => {
        ...
        setbuttonval('Update Particular Master');
        setSubmitVal('Update');
    }
    ...
    const columns = useMemo(
        () => [
            ...
            {
                header: 'ParticularName',
                accessorKey: 'particularName',
                disableFilters: false,
                enableColumnFilter: false,
            },
            {
                header: 'ParticularCode',
                accessorKey: 'particularCode',
                disableFilters: false,
                enableColumnFilter: false,
            },
            {
    ...
    const validation = useFormik({
        ...
        initialValues: {
            ...
            particularName: edit_items.particularName || "",
            particularCode: edit_items.particularCode || "",
            isDisabled: false,
            requestUserName: ''
        },
        validationSchema: Yup.object({
            
            particularName: Yup.string().required("Please Enter Particular Name"),
            particularCode: Yup.string().required("Please Enter Particular Code"),
        }),
        onSubmit: async (values) => {
            let response;
            try {
                if (edit_update === true) {
                    response = updateParticularMaster({

                        particularId: edit_items.particularId,
                        particularName: values.particularName,
                        particularCode: values.particularCode,
                        clientId:sessClient,
                        countryId:scountry,
                        isDisabled: !checked,
    ...
    const filter = (e) => {
        ...
        if (keyword !== '') {
            const results = filterData?.filter((d) => {
                return d.particularName.toLowerCase().includes(keyword.toLowerCase());
            });
            setData(results);
        } else {
            setData(databk);
    ...
    document.title = "Particular Master | eSoft Digital Platform";
    ...
    return (
        <React.Fragment>
            ...
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Particular Master" pageTitle="Master" />
                    <Row>
                        <Col lg={12}>
                            <Card>
                                        ...
                                        <Col className="col-sm-2">
                                            <div className="search-box ms-2">
                                                <input
                                                    ...
                                                    id="searchResultList"
                                                    placeholder="Search for Particulars..."
                                                    onKeyUp={(e) => filter(e)}
                                                />
            ...
            {/* Add Modal */}
            <Modal className="modal-dialog modal-dialog-centered" isOpen={modal_list} toggle={() => { tog_list(); }} centered >
                ...
                <form className="tablelist-form"
                    ...
                    <ModalBody>
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="particularName">Particular Name</Label>
                                    <Input
                                        name="particularName"
                                        ...
                                        value={validation.values.particularName || ""}
                                        invalid={
                                            validation.touched.particularName &&
                                                validation.errors.particularName
                                                ? true
                                                : false
                                        }
                                    />
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="particularCode">Particular Code</Label>
                                    <Input
                                        name="particularCode"
                                        ...
                                        onChange={validation.handleChange}
                                        onBlur={validation.handleBlur}
                                        value={validation.values.particularCode || ""}
                                        invalid={
                                            validation.touched.particularCode &&
                                                validation.errors.particularCode
                                                ? true
                                                : false
                                        }
                                    />
...
export default ParticularMaster;
