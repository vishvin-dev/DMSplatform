...
import {
    ...
    putEsApplicationUpdate,
    getEsApplicationById,
    getAllEsApplication
...
const [buttonval, setbuttonval] = useState('Add Application');
...
//modal toggle
const tog_list = async (values) => {

    setbuttonval('Add Application');
...
const updateRow = async (item) => {
    ...
    setbuttonval('Update Application');
    setSubmitVal('Update');
    setmodal_list(!modal_list);
}
...
const validation = useFormik({
    ...
    initialValues: {
        ...
        applicationName: edit_items.applicationName || "",
        applicationCode: edit_items.applicationCode || "",
        isDisabled: false,
        requestUserName: ''
    },
    validationSchema: Yup.object({
        applicationName: Yup.string().required("Please Enter Your Application Name"),
        applicationCode: Yup.string().required("Please Enter Your Application Code")
    ...
    onSubmit: async (values) => {
        ...
        try {
            if (edit_update === true) {
                response = putEsApplicationUpdate({
                    applicationId: edit_items.applicationId,
                ...
                response = postApplicationCreate({
                    applicationName: values.applicationName,
                    applicationCode: values.applicationCode,
                    isDisabled: false,
                    requestUserName: username
            ...
            if (data) {
                let response = getAllEsApplication();
                var allApps = await response;
                setedit_update(false);
                setedit_items('')
                setbuttonval('Add Application')
...
const filter = (e) => {
    ...
    if (keyword !== '') {
        const results = filterData?.filter((d) => {
            return d.applicationName.toLowerCase().includes(keyword.toLowerCase());
        });
        setData(results);
    } else {
        setData(databk);
...
document.title = "Application Creation | eSoft Digital Platform";
...
return (
    <React.Fragment>
        ...
        <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Application Creation" pageTitle="Applications" />
                    <Row>
                        <Col lg={12}>
                            <Card className="card">
                                ...
                                <CardBody>
                                    <Row className="g-4 mb-3">
                                        ...
                                        <Col className="col-sm-auto justify-content-sm-end">
                                            <div>
                                                <Button color="primary" className="add-btn me-1 btn-primary" onClick={() => tog_list()} id="create-btn"><i className="ri-add-line align-bottom me-1"></i> Add</Button>
                                            </div>
                                        </Col>
                ...
                <form className="tablelist-form"
                    ...
                    <ModalBody>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    ...
                                    <Input
                                        ...
                                        value={validation.values.applicationName || ""}
                                        invalid={
                                            validation.touched.applicationName &&
                                                validation.errors.applicationName
                                                ? true
                                                : false
                                        }
                                    />
                        ...
                        <Row>
                            ...
                            <Col md={10}>
                                <FormGroup className="mb-10">
                                    <Label htmlFor="validationCustom01">Application Code</Label>
                                    <Input
                                        ...
                                        onChange={validation.handleChange}
                                        onBlur={validation.handleBlur}
                                        value={validation.values.applicationCode || ""}
                                        invalid={
                                            validation.touched.applicationCode &&
                                                validation.errors.applicationCode
                                                ? true
                                                : false
                                        }
                                    />
