...
// New component for Document Tabs in preview modal
const DocumentTabs = React.memo(({ documents, activeDocument, onDocumentSelect }) => {
    return (
        <div className="document-versions-list">
            {documents.map((doc, index) => (
                <div 
                    key={doc.Version_Id} 
                    className={`document-version-item ${activeDocument?.Version_Id === doc.Version_Id ? 'active' : ''}`}
                    onClick={() => onDocumentSelect(doc)}
                >
                    <div className="version-header">
                        <i className="ri-file-text-line me-2"></i>
                        <strong>{doc.VersionDocumentName || doc.DocumentName}</strong>
                    </div>
                    <div className="version-details">
                        <small className="text-muted">
                            {new Date(doc.UploadedAt || doc.CreatedAt).toLocaleDateString()} - {doc.VersionMetaTags || 'Updated Document'}
                        </small>
                        <br />
                        <small className="version-label">Version: {doc.VersionLabel}</small>
                        {activeDocument?.Version_Id === doc.Version_Id && (
                            <Badge color="primary" className="ms-2 current-badge">
                                Current
                            </Badge>
                        )}
                    </div>
...
const QCViewDocuments = () => {
    ...
    const [documents, setDocuments] = useState([]);
    const [filteredDocuments, setFilteredDocuments] = useState([]);
    const [groupedDocuments, setGroupedDocuments] = useState([]); // New state for grouped documents
    const [loading, setLoading] = useState(false);
    const [previewModal, setPreviewModal] = useState(false);
    const [currentDoc, setCurrentDoc] = useState(null);
    const [currentDocumentGroup, setCurrentDocumentGroup] = useState([]); // For multiple versions
    const [actionLoading, setActionLoading] = useState(false);
    const [rejectionModal, setRejectionModal] = useState(false);
    const [localRejectionReason, setLocalRejectionReason] = useState('');
    const [searchValue, setSearchValue] = useState('');
    ...
    const groupDocumentsByDocumentId = (docs) => {
        const grouped = {};
        docs.forEach(doc => {
            if (!grouped[doc.DocumentId]) {
                grouped[doc.DocumentId] = [];
            }
            grouped[doc.DocumentId].push(doc);
        });
        return grouped;
    };

    // Create table data from grouped documents (showing one row per DocumentId)
    const createTableDataFromGroups = (groupedDocs) => {
        return Object.values(groupedDocs).map(group => {
            // Use the first document in the group for common fields
    ...
    const fetchDocuments = async (tab, userData) => {
        ...
        try {
            ...
            if (response.status === "success") {
                ...
                // Group documents by DocumentId for table display
                const grouped = groupDocumentsByDocumentId(mappedDocs);
                setGroupedDocuments(grouped);
                
                // Create table data (one row per DocumentId)
                const tableData = createTableDataFromGroups(grouped);
                setFilteredDocuments(tableData);
                
                if (tab === 'pending') {
                    setDataBk(tableData); // Update the backup for pending docs
                }
            } else {
                setResponse(response.message || `Failed to fetch ${tab} documents`);
                setErrorModal(true);
    ...
    const handleSearch = (e) => {
        ...
        const sourceData = activeTab === 'pending' ? databk : filteredDocuments;
    ...
    // Handle file select for preview - Modified to handle grouped documents
    const handleFileSelect = async (doc) => {
        // If this is a grouped document (multiple versions), show all versions
        if (doc._isGroup && doc._allVersions) {
            setCurrentDocumentGroup(doc._allVersions);
            // Set the first document as active for preview
            const firstDoc = doc._allVersions[0];
            setCurrentDoc(firstDoc);
            await loadPreviewContent(firstDoc);
        } else {
            // Single document
            setCurrentDocumentGroup([doc]);
            setCurrentDoc(doc);
            await loadPreviewContent(doc);
        }
        setPreviewModal(true);
    };

    // Load preview content for a specific document
    const loadPreviewContent = async (doc) => {
        setPreviewLoading(true);
        setPreviewContent(null);
        setPreviewError(null);
        ...
        try {
            ...
            setPreviewContent({
                url: fileUrl,
                type: 'application/pdf',
                name: doc.VersionDocumentName || doc.DocumentName,
                blob: blobToView
            });
    ...
    // Handle document selection from tabs in preview modal
    ...
    try {
        ...
        if (response.status === "success") {
            ...
            setResponse(response.message || 'Document approved successfully');
            setSuccessModal(true);

            // Refresh the data
            await handleRefresh();
        } else {
            setResponse(response.message || 'Failed to approve document');
            setErrorModal(true);
        }
    } catch (error) {
        console.error('Approve error:', error);
        setResponse(error.message || 'Failed to approve document');
        setErrorModal(true);
    } finally {
        setActionLoading(false);
    }
    ...
    const renderTableRows = () => {
        ...
        return paginatedData.map((doc, index) => (
            <tr key={doc.DocumentId || doc.Version_Id || index}>
                <td>
                    <div className="d-flex align-items-center">
                        {getFileIcon(doc.DocumentName)}
                        <span title={doc.DocumentName}>
                        ...
                        {doc._isGroup && doc._groupCount > 1 && (
                            <Badge color="info" className="ms-2">
                                {doc._groupCount} Files
                            </Badge>
                        )}
                        {doc.Status === 'Approved' && (
                            <Badge color="success" className="ms-2">Approved</Badge>
        ...
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                                                    ...
                                                    <h5 className="mb-0">
                                                        ...
                                                        {groupedDocuments && Object.keys(groupedDocuments).length > 0 && (
                                                            <small className="text-muted ms-2">
                                                                ({Object.keys(groupedDocuments).length} unique documents)
                                                            </small>
                                                        )}
                                                    </h5>
                                                </Col>
                    ...
                    {/* Document Preview Modal - TWO COLUMN LAYOUT WITH COMBINED INFORMATION */}
                    <Modal
                        isOpen={previewModal}
                        toggle={closePreview}
                        size="sm"
                    ...
                    >
                        <ModalHeader className="bg-primary text-white p-3" toggle={closePreview}>
                            <span className="modal-title text-white">
                                <i className="ri-file-text-line me-2"></i>
                                Document Preview - {currentDoc?.VersionDocumentName || currentDoc?.DocumentName || 'Document'}
                            </span>
                        </ModalHeader>
                        <ModalBody style={{
                            maxHeight: '80vh',
                            ...
                            <Row className="h-100">
                                {/* Column 1: Document Versions & Combined Information */}
                                <Col lg={4} className="mb-3">
                                    <div className="d-flex flex-column h-100 gap-3">
                                        ...
                                        <Card className="shadow-sm">
                                            <CardHeader className="bg-light p-3">
                                                <h6 className="mb-0 d-flex align-items-center">
                                                    <i className="ri-folder-upload-line me-2"></i>
                                                    Uploaded Documents
                                                    {currentDocumentGroup.length > 0 && (
                                                        <Badge color="primary" className="ms-2">
                                                            {currentDocumentGroup.length}
                                                        </Badge>
                                                    )}
                                                </h6>
                                            </CardHeader>
                                            <CardBody className="p-3" style={{ overflowY: 'auto', maxHeight: '200px' }}>
                                                {currentDocumentGroup.length > 1 ? (
                                                    <DocumentTabs
                                                        documents={currentDocumentGroup}
                                                    ...
                                                    <div className="single-document">
                                                        <div className="document-item active">
                                                            <div className="version-header">
                                                                <i className="ri-file-text-line me-2"></i>
                                                                <strong>{currentDoc?.VersionDocumentName || currentDoc?.DocumentName || 'Test Document'}</strong>
                                                            </div>
                                                            <div className="version-details">
                                                                <small className="text-muted">
                                                                    {formatDate(currentDoc?.UploadedAt || currentDoc?.CreatedAt) || '20/11/2025 11:00:46 am'} - {currentDoc?.VersionMetaTags || 'Updated Policy'}
                                                                </small>
                                                                <br />
                                                                <small className="version-label">Version: {currentDoc?.VersionLabel || 'v2'}</small>
                                                                <Badge color="primary" className="ms-2 current-badge">
                                                                    Current
                                                                </Badge>
                                                            </div>
                                        ...
                                        {/* Combined Consumer & Document Information */}
                                        <Card className="shadow-sm flex-grow-1">
                                            ...
                                            <CardBody className="p-3" style={{ overflowY: 'auto', maxHeight: '300px' }}>
                                                <div className="combined-info-details">
                                                    ...
                                                    <div className="info-section mb-3">
                                                        ...
                                                        <div className="info-row compact">
                                                            ...
                                                            <span>{currentDoc?.Account_Id}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Name:</strong>
                                                            <span>{currentDoc?.consumer_name}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Address:</strong>
                                                            <span>{currentDoc?.consumer_address}</span>
                                                        </div>
                                                        {/* <div className="info-row compact">
                                                            <strong>Phone:</strong>
                                                            <span>{currentDoc?.rr_no}</span>
                                                        </div> */}
                                                        <div className="info-row compact">
                                                            <strong>RR No:</strong>
                                                            <span>{currentDoc?.rr_no}</span>
                                                    ...
                                                    <div className="info-section">
                                                        ...
                                                        <div className="info-row compact">
                                                            ...
                                                            <span>{currentDoc?.VersionDocumentName || currentDoc?.DocumentName || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Uploaded By:</strong>
                                                            <span>{currentDoc?.CreatedByUserName || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Upload Date:</strong>
                                                            <span>{formatDate(currentDoc?.UploadedAt || currentDoc?.CreatedAt) || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Version:</strong>
                                                            <span>{currentDoc?.VersionLabel || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Division:</strong>
                                                            <span>{currentDoc?.division || currentDoc?.DivisionName || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Sub Division:</strong>
                                                            <span>{currentDoc?.sub_division || currentDoc?.SubDivisionName || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Section:</strong>
                                                            <span>{currentDoc?.section || currentDoc?.SectionOfficeName || 'N/A'}</span>
                                                        </div>
                                                        <div className="info-row compact">
                                                            <strong>Meta Tags:</strong>
                                                            <span>{currentDoc?.VersionMetaTags || currentDoc?.MetaTags || 'N/A'}</span>
                                                        </div>
                                                        {currentDoc?.Status === 'Rejected' && (currentDoc?.RejectionReason || currentDoc?.RejectionComment) && (
                                                            <div className="info-row compact">
                                                                <strong>Rejection Reason:</strong>
                                                                <span className="text-danger">{currentDoc?.RejectionReason || currentDoc?.RejectionComment}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                <Col lg={8} className="mb-3">
                                    <Card className="shadow-sm h-100">
                                        ...
                                        <CardBody className="p-0 d-flex flex-column" style={{ minHeight: '500px' }}>
                                            {previewLoading ? (
                                                <div className="text-center h-100 d-flex flex-column justify-content-center align-items-center flex-grow-1">
                                                    <div className="spinner-border text-primary" role="status">
                                                        <span className="visually-hidden">Loading...</span>
                                                    </div>
                                                    <p className="mt-2">Loading document content...</p>
                                                ...
                                                <div className="text-center text-muted py-5 h-100 d-flex flex-column justify-content-center fade-in flex-grow-1">
                                                    <i className="ri-file-line display-4"></i>
                                                    <h5 className="mt-3">No document selected</h5>
                                                    <p>Select a file from the list to preview it here</p>
                                                </div>
            ...
            <style>
                ...
                .document-version-item.active .text-muted {
                ...
                .single-document .document-item .text-muted {
                    color: #e9ecef !important;
                }

                .single-document .document-item .version-label {
                    color: #e9ecef;
                }
                ...
                @media (max-width: 991px) {
                    ...
                    .preview-layout .col-lg-4,
                    .preview-layout .col-lg-8 {
                        margin-bottom: 20px;
                    }
