    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            if (usernm) {
                ...
                flagIdFunction({ flagId: 6, requestUserName: usernm }).then(setRoles);
                flagIdFunction({ flagId: 7, requestUserName: usernm }).then(setDocumentCategory);
            }
        ...
        loadInitialData();
    }, [loadDropdownDataFromSession, flagIdFunction]);
    ...
    return (
        <div className="page-content">
            ...
            <Container fluid>
                ...
                {/* REST OF THE MODALS REMAIN THE SAME - Add/Edit Document Modal, Approved Modal, Rejected Modal, Re-upload Modal */}
                {/* Due to character limits, the modals code remains exactly as in your original code */}
                {/* Add/Edit Document Modal */}
                <Modal isOpen={modalOpen} toggle={() => setModalOpen(!modalOpen)} size="lg">
                    <ModalHeader className="bg-primary text-white p-3" toggle={() => setModalOpen(false)}>
                        <span className="modal-title text-white">{editMode ? 'Edit Document' : 'Add New Document'}</span>
